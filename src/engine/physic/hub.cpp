        ...
        if( l_body->getShape()) {
            for( uint32_t n = 0; n < p_bodys.size(); n++) {
                if( testAABBAABB(   l_body->getPosition() + l_body->getShape()->getOffset() + (l_body->getVelocity() * dt), l_body->getShape()->getAABB(),
                    ...
                    float collisiontime = sweptAABB(    l_body->getPosition() + l_body->getShape()->getOffset(), l_body->getShape()->getAABB(),
                                                        p_bodys[n]->getPosition() + p_bodys[n]->getShape()->getOffset(), p_bodys[n]->getShape()->getAABB(),
                                                        (l_body->getVelocity() * dt),
                                                        &l_normal);
                    
                    // Auf normal reagieren
                    l_body->addPosition( (l_body->getVelocity() * dt) * collisiontime);
                    l_body->setVelocity( {  l_normal.x?0.f:l_body->getVelocity().x,
                                            l_normal.y?0.f:l_body->getVelocity().y} );
                }
            }
        }
